...
void update(char* nume_fisier)
{
    ...
    sprintf(nr, "%d", x);
    strcpy(np, "");
    strcat(np, strcat(strdup("Produs"),nr));
    ...
    y = (Produs*) bsearch (&np, &p, 100, sizeof(Produs), compare);
    if(y == NULL)
    ...
    else
    {
        ...
        if(strcmp(np1, "\n") != 0)
        {
            ...
            for(j=0;j<100;j++)
            {
                if(strcmp(p[j].nume_produs, y -> nume_produs) == 0)
                {
                    strcpy(p[j].nume_produs, "");
                    strcat(p[j].nume_produs, n1);
                    p[j].cantitate = n2;
                    p[j].pret_produs = n3;
                }
            }
            FILE *f = fopen(NF, "wb");
            fwrite(p, 100, sizeof(Produs), f);
            fclose(f);
        }
    }
